...
void main() async
{
  ...
  /// https://hackage.haskell.org/package/path-0.9.6/docs/Path-Posix.html#:~:text=A%20path%20is%20represented%20by%20a%20number%20of%20path%20components%20separated%20by%20a%20path%20separator%20which%20is%20a%20/%20on%20POSIX%20systems%20and%20can%20be%20a%20/%20or%20%5C%20on%20Windows.
    ...
    String slashBasedFilePath = r"test\qa_utils\test_semi_automations_utils\visual_testing_helper_generation.dart";
  // String slashBasedFilePath = "test/common_widgets/interaction_and_inputs/custom_language_switcher_visual_testing.dart";
  ...
  File file = File(slashBasedFilePath);
  String content = file.readAsStringSync();
  if (content.contains(delimiterLine))
  {
    logger.shout("The delimiter line was found in $slashBasedFilePath");
    logger.shout("");
    // exit(0);    // Gives an impression of program crash
  }
  else 
  {
    logger.info("The delimiter line wasn't found in $slashBasedFilePath");
    logger.info("Addition to the file");
    String comment = delimiterLine + eol +
                      commentBegin + slashBasedFilePath + commentEndLinux + eol +
                      commentBegin + slashBasedFilePath + commentEndMacOS  + eol +
                      commentBegin + slashBasedFilePath + commentEndWindows + eol +
                      delimiterLine + eol;    
    await fileUtils.appendTextInFront(filePath: slashBasedFilePath,text: comment);
